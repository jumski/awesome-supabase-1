...
- [Edge Worker](https://pgflow.dev) - Open-source serverless task queue worker that runs on Supabase Edge Functions (Background Tasks) and Supabase Queues. It simplifies consuming the queues and adds useful features like concurrency control, retries, and observability. Designed as a **standalone solution**, it also serves as the foundation for the upcoming **pgflow** workflow engine.
